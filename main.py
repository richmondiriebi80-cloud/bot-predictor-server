...
# 3. Chargement sécurisé du modèle XGBoost
model = xgb.Booster()
if os.path.exists("modele_fifa_xgboost.json"):
    try:
        model.load_model("modele_fifa_xgboost.json")
    except Exception as e:
        print(f"Erreur chargement modèle: {e}, mode simulation actif.")
else:
    print("Mode simulation activé (modele_fifa_xgboost.json introuvable).")

# 4. Modèle de données pour l'analyse
...
# 6. Route d'analyse XGBoost complète
